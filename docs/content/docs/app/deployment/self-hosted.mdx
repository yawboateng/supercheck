  ...
  <Tab value="Quick Start">
    <Steps>
      <Step>
        ...
        <Callout type="info">
        A **Linux server** is required. Supercheck uses sandboxed Kubernetes pods for test execution, which is only available on Linux.
        </Callout>
      ...
      <Step>
        ...
        This sets up the sandboxed execution environment for Playwright and K6 jobs.
      </Step>
...
<Callout type="info">
`RUNNING_CAPACITY` and `QUEUED_CAPACITY` are App-side gates. `WORKER_REPLICAS` controls actual worker containers. Keep `RUNNING_CAPACITY` equal to total replicas.
</Callout>
