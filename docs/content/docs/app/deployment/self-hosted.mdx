...
<Callout type="info">
**gVisor Sandbox:** `setup-k3s.sh` installs K3s, installs [gVisor](https://gvisor.dev), registers `runsc` for Docker Compose workers, creates the `gvisor` RuntimeClass, provisions the `supercheck-execution` namespace, and writes `/etc/rancher/k3s/supercheck-worker.kubeconfig` for the Compose worker. Keep `runtime: runsc` on the worker container as well so both the control plane and per-run execution Jobs consistently use gVisor.
</Callout>
