...
The examples below use the built-in `us-east`, `eu-central`, and `asia-pacific` location codes, but any enabled custom location follows the same routing pattern.
...
<Callout type="info">
K6 **job** executions are automatically routed to the default location configured in the Super Admin panel. Ensure at least one worker is running for that location. Playground K6 tests respect the user's location selection.
</Callout>
...
All enabled locations are shown in user-facing location pickers, each with an online/offline indicator based on worker heartbeats. If a region is enabled in Super Admin but no worker is currently advertising `k6-{code}` / `monitor-{code}`, it appears with an offline badge — but it is **not removed** from the configuration. This prevents transient worker outages from silently stripping regions from monitor configs. When a monitor or K6 job runs, only locations with an active worker receive jobs; offline locations are skipped gracefully. The run only fails if **no** configured location has an active worker.
